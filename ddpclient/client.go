...
func (ddpClient *DDPClient) LoginUser() {
	...
	//fmt.Println("MY_USERNAME:" + os.Getenv("MY_USERNAME"))
	//fmt.Println("MY_PASSWORD:" + os.Getenv("MY_PASSWORD"))
...
func (ddpClient *DDPClient) ListenRead() {
	...
	fmt.Println("Listening read from server")
	for {
		var msg string
		...
		case strings.Contains(msg, "server_id"):
			...
			fmt.Println("ServerId: " + data.ServerId)
		...
		case strings.Contains(msg, "connected"):
			...
			fmt.Println("Connected to ddp server with session: " + data.Session)
		...
		case strings.Contains(msg, "failed"):
			...
			fmt.Println(data.Msg + ", server requires version: " + data.Version)
		// <- Establishing a DDP Connection
